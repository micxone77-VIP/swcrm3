alter table public.campaigns add column if not exists target_countries text[] not null default array['MY'];
...
begin
  ...
  countries := coalesce(new.target_countries, array['MY']);
  if coalesce(array_length(tiers, 1), 0) = 0 or coalesce(array_length(countries, 1), 0) = 0 then
    return new;
  end if;
  ...
  where coalesce(v.is_excluded, false) = false
    ...
    and (
      case upper(coalesce(v.currency, ''))
        when 'MYR' then 'MY'
        when 'SGD' then 'SG'
        when 'KHUSD' then 'KH'
        else null
...
after insert or update of target_tier, auto_enroll_tiers, target_countries on public.campaigns
for each row
