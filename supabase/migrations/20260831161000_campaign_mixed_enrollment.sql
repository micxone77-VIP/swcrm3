...
create index if not exists campaign_players_campaign_enrollment_source_idx
  on public.campaign_players (campaign_id, enrollment_source);
...
as $$
begin
  if coalesce(array_length(new.auto_enroll_tiers, 1), 0) = 0 then
    return new;
  end if;
  ...
  where coalesce(v.is_excluded, false) = false
    and v.tier = any(new.auto_enroll_tiers)
...
after insert or update of auto_enroll_tiers on public.campaigns
for each row
