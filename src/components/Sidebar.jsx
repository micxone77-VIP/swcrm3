...
// 5 domains per spec section 3.1
const NAV_GROUPS = [
  {
    key: 'command',
...
function NavItem({ to, icon, label }) {
  return (
    <NavLink to={to} style={({ isActive }) => ({
      ...
      background: isActive ? 'rgba(255,106,0,.12)' : 'transparent',
      textDecoration: 'none',
  ...
  return (
    <div style={{ marginBottom: 4 }}>
      <button onClick={() => setOpen(o => !o)} style={{
        ...
        padding: '5px 12px 5px 10px', borderRadius: 7, border: 'none',
        background: 'transparent', cursor: 'pointer', gap: 8,
        marginBottom: 2,
      }}>
        <span style={{ display: 'flex', alignItems: 'center', gap: 7 }}>
          <span style={{ fontSize: 13 }}>{group.icon}</span>
          <span style={{ fontSize: 11, fontWeight: 700, color: 'var(--disabled)', letterSpacing: '.6px', textTransform: 'uppercase' }}>
            {group.label}
          </span>
        </span>
        <span style={{
          fontSize: 9, color: 'var(--disabled)',
      ...
      {open && (
        <div style={{ paddingLeft: 4 }}>
          {visible.map(n => <NavItem key={n.to} to={n.to} icon={n.icon} label={n.label} />)}
        </div>
      )}
    </div>
  )
  ...
  return (
    <aside style={{
      ...
      <div style={{
        ...
        <div>
          <div style={{ fontSize: 15, fontWeight: 700, color: 'var(--text)', display: 'flex', alignItems: 'center', gap: 6 }}>
            <span style={{ color: 'var(--brand)', fontWeight: 900 }}>Sure</span>Win CRM
          </div>
          <div style={{ fontSize: 11, color: 'var(--muted)', marginTop: 2 }}>VIP Operations Command Center</div>
        </div>
        <NotificationBell />
      </div>
      ...
      <nav style={{ flex: 1, padding: '10px 8px', overflowY: 'auto' }}>
        {NAV_GROUPS.map(g => (
          <NavGroup key={g.key} group={g} role={role} />
        ))}
      </nav>
      ...
      <div style={{ padding: '12px 14px', borderTop: '1px solid var(--border)' }}>
        ...
        <div style={{ display: 'flex', gap: 5, marginBottom: 10 }}>
          {['en', 'zh'].map(l => (
            <button key={l} onClick={() => setLang(l)} style={{
              flex: 1, background: lang === l ? 'var(--brand)' : 'var(--surface2)',
              color: lang === l ? '#fff' : 'var(--muted)',
              border: `1px solid ${lang === l ? 'var(--brand)' : 'var(--border)'}`,
        ...
        <button onClick={handleSignOut} style={{
          ...
          color: 'var(--muted)', padding: '7px', borderRadius: 6,
